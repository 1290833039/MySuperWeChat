...
//					unreadAddressLable.setText(String.valueOf(count));
	...
	public class MyContactListener implements EMContactListener {
				...
				if (isAdd){
					try {
						...
						executeRequest(new GsonRequest<Contact>(path,Contact.class,
								responAddContactListener(),errorListener()));
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
		...
		private Response.Listener<Contact> responAddContactListener() {
				...
				for (String name : toDeleteUserNames){
					...
					try {
						...
						executeRequest(new GsonRequest<Boolean>(path,Boolean.class,
								responseDeleteListener(name),errorListener()));
		...
		private Response.Listener<Boolean> responseDeleteListener(final String name) {
			return new Response.Listener<Boolean>() {
				@Override
				public void onResponse(Boolean response) {
					if (response){
						SuperWeChatApplication.getInstance().getUserList().remove(name);
						SuperWeChatApplication.getInstance().getContactList().remove(name);
						MainActivity.this.sendStickyBroadcast(new Intent("update_contact_list"));
					}
				}
			};
		}
